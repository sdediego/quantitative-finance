function [price, error] = priceDownAndOutBarrierCallOption_MC(S0,K,r,T,sigma,H,N,M)
% priceDownAndOutBarrierCallOption_MC: Down-and-out barrier option price. 
% 
% SINTAXIS:  
%  [price, error] = priceDownAndOutBarrierCallOption_MC(S0,K,r,T,sigma,H,N,M)
%
...
%  H = 80; N = 1000; M = 10000;
%  priceDownAndOutBarrierCallOption_MC(S0,K,r,T,sigma,H,N,M)
%
dt = T/N;
X = randn(M,N);
S = cumprod([S0*ones(M,1) exp((r-sigma^2/2)*dt + sigma*sqrt(dt)*X)],2);
barrier_down = ones(size(S)).*H;
Mat_ind = S > barrier_down;  
